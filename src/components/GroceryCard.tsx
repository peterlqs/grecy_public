                ...
                <div className="text-xs">
                  ...
                  {item.ingredients.split(',').map((ingredient, index) => {
                    ...
                    return (
                      ...
                      >
                        {trimmedIngredient}
                        {item.ingredients && index < item.ingredients.split(',').length - 1 ? ', ' : ''}
                      </span>
                    );
                  })}
                  <br />
                  <strong>Allergens:</strong>{" "}
{
  item.allergen?.split(',').map((allergen, index) => (
    <span key={index}>
      {allergen.trim().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')}
           ...
           <div className="relative">
             <div
               className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white ${
                 ...
                 item.nutriScore === 'B' ? 'bg-green-700' :
             ...
             <span className="absolute -top-3 left-1/2 transform -translate-x-1/2 text-[8px]">Nutri</span>
           </div>
         )}
         {item.novaGroup && (
           <div className="relative">
             <div
               className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white ${
                 item.novaGroup === '1' ? 'bg-green-500' :
                 item.novaGroup === '2' ? 'bg-yellow-500' :
                 item.novaGroup === '3' ? 'bg-orange-500' :
                 'bg-red-500'
               }`}
             >
               {item.novaGroup}
             </div>
             <span className="absolute -top-3 left-1/2 transform -translate-x-1/2 text-[8px]">Nova</span>
           </div>
         )}
       </div>
        {!item.ingredients && (
