  ...
  return (
    ...
    >
      ...
      <div className={clsx(className, "w-full p-6 container-narrow")}>
        {title ? (
          <h2 className="text-3xl font-semibold leading-tight max-w-xs sm:max-w-none md:text-4xl lg:text-5xl fancyHeading mb-4 text-center max-w-[40ch]">
            {title}
          </h2>
        ) : (
          titleComponent
        ...
        {description ? (
          <p className="mt-6 md:text-xl text-center max-w-lg">{description}</p>
        ) : (
          descriptionComponent
        )}

        <div className="mt-6 flex flex-wrap gap-4 w-full items-center justify-center">
          {ctaLabel ? (
